...
int create_board_window(int dim_x, int dim_y){
   ...
   if( !( IMG_Init( imgFlags ) & imgFlags ) ){
 	...
 	SDL_Surface* loadedSurface = IMG_Load("Images/monster.png");
   if( loadedSurface == NULL ){
       printf( "Unable to load image %s! SDL_image Error: %s\n", "Images/monster.png", IMG_GetError() );
 			exit(-1);
   } else {
	 ...
	 //load pacman
	 loadedSurface = IMG_Load("Images/pacman.png");
 	if( loadedSurface == NULL ){
 			printf( "Unable to load image %s! SDL_image Error: %s\n", "Images/pacman.png", IMG_GetError() );
 		 exit(-1);
 	} else {
	...
	//load powerpacman
	loadedSurface = IMG_Load("Images/powerpacman.png");
	 if( loadedSurface == NULL ){
		 printf( "Unable to load image %s! SDL_image Error: %s\n", "Images/powerpacman.png", IMG_GetError() );
		exit(-1);
 	} 
	 else {
	...
	//load lemon
	loadedSurface = IMG_Load("Images/lemon.png");
 if( loadedSurface == NULL ){
		 printf( "Unable to load image %s! SDL_image Error: %s\n", "Images/lemon.png", IMG_GetError() );
		exit(-1);
 } else {
 ...
 //load brick
 loadedSurface = IMG_Load("Images/brick.png");
if( loadedSurface == NULL ){
		printf( "Unable to load image %s! SDL_image Error: %s\n", "Images/brick.png", IMG_GetError() );
	 exit(-1);
} else {
...
//load brick
loadedSurface = IMG_Load("Images/cherry.png");
if( loadedSurface == NULL ){
	 printf( "Unable to load image %s! SDL_image Error: %s\n", "Images/cherry.png", IMG_GetError() );
	exit(-1);
} else {
